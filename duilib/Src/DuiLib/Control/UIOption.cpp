...
namespace DuiLib
{
	...
	void COptionUI::Selected(bool bSelected,bool bNotify)
	{
//		if( m_bSelected == bSelected ) return;
	...
	void COptionUI::SetSelectedFont(int nSelectedFont)
	{
		m_iSelectedFont=nSelectedFont;
	}
	...
	void COptionUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		if( _tcscmp(pstrName, _T("group")) == 0 ) SetGroup(pstrValue);
			...
			int nFont=_ttoi(pstrValue);
			SetSelectedFont(nFont);
		}
		else if( _tcscmp(pstrName, _T("showforeimage")) == 0 ) {
			m_bShowForeImage=_tcscmp(pstrValue, _T("true")) == 0;
		}
...
Label_ForeImage:
		if ( IsSelected() && !m_sSelectedForeImage.IsEmpty() && m_bShowForeImage)
		{
			if( !DrawImage(hDC, (LPCTSTR)m_sSelectedForeImage) ) m_sSelectedForeImage.Empty();
		}
		else if(  !m_sForeImage.IsEmpty() && m_bShowForeImage ) 
		{
	...
	void COptionUI::PaintText(HDC hDC)
	{
		if( (m_uButtonState & UISTATE_SELECTED) != 0 )
		{
			...
			{
				int nFont=m_iSelectedFont>=0 ? m_iSelectedFont: m_iFont;
	...
	void COptionUI::SetIsShowForeImage(bool bShowForeImage)
	{
		m_bShowForeImage=bShowForeImage;
		Invalidate();
	}
}