...
namespace DuiLib
{
	...
	{  
		...
		{  
			...
			{  
				pCallback->OnDocumentReady(pWebkitUI);  
				bOver=true;  
			}  
			if ( pWebView->isLoadFailed() )  
			{  
				pCallback->OnLoadFailed(pWebkitUI);  
				return 1;  
			}  
			::Sleep(30);  
		}  
		if ( pWebView->isLoadComplete() )  
			pCallback->OnLoadComplete(pWebkitUI);  
		return 0;  
	}  
	...
	{
		if( _tcscmp(pstrName, _T("WkeWebkitUI")) == 0 ) return static_cast<CWkeWebkitUI*>(this);
	...
	void CWkeWebkitUI::SetURL(LPCTSTR strValue)
	{
		StopCheckThread(); 
		if(m_pWindow)
		{
			m_pWindow->m_strUrl = strValue;
			if(m_pWindow->m_pWebView)
			{
				m_pWindow->m_pWebView->loadURL(strValue);
				StartCheckThread();  
			}
		}	
	}

	void CWkeWebkitUI::SetFile( LPCTSTR strValue)
	{
		StopCheckThread(); 
		if(m_pWindow)
		{
			...
			if(m_pWindow->m_pWebView)
			{
				m_pWindow->m_pWebView->loadFile(strValue);
				StartCheckThread();  
			}		
		}	
	}

	LPCTSTR CWkeWebkitUI::RunJS(LPCTSTR strValue)
	{
		if(m_pWindow && m_pWindow->m_pWebView)
		{
			jsValue jsRet = m_pWindow->m_pWebView->runJS(strValue);
	...
	{
		if (m_pWindow && m_pWindow->m_pWebView && m_pWindow->m_pWebView->canGoBack())
			m_pWindow->m_pWebView->goBack();
	}

	void CWkeWebkitUI::GoForward()
	{
		if (m_pWindow && m_pWindow->m_pWebView && m_pWindow->m_pWebView->canGoForward())
