...
namespace DuiLib
{
	...
	UINT CGifAnimUI::GetControlFlags() const
	{
		return (IsKeyboardEnabled() ? UIFLAG_TABSTOP : 0) | (IsEnabled() && m_bCanClick ? UIFLAG_SETCURSOR : 0);
	}
	...
	void CGifAnimUI::DoEvent( TEventUI& event )
	{
		...
		{
			if( ::PtInRect(&m_rcItem, event.ptMouse) && m_bCanClick)
			{
				if( m_pManager != NULL )
				{
	...
	void CGifAnimUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if(_tcscmp(pstrName, _T("canclick")) == 0){
			m_bCanClick=_tcsicmp(pstrValue, _T("true")) == 0;
		}
		else
	...
	Gdiplus::Image* CGifAnimUI::LoadGifFromFile(LPCTSTR pstrGifPath)
	{
		...
		do 
		{
			...
			if( CPaintManagerUI::GetResourceZip().IsEmpty() ) {
				...
				if( dwSize == 0 || dwSize==INVALID_FILE_SIZE) break;
