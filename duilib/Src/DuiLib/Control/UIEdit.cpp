...
namespace DuiLib
{
	...
	LRESULT CEditWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		{
			m_pOwner->GetManager()->SendNotify(m_pOwner, DUI_MSGTYPE_ENTERTAB);
		}
	...
	{
		m_pOwner->GetManager()->SendNotify(m_pOwner, DUI_MSGTYPE_EDIT_KILLFOCSE);
	...
	LRESULT CEditWnd::OnEditChanged(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& /*bHandled*/)
	{
		...
		m_pOwner->CControlUI::SetText(pstr);
	...
	void CEditUI::SetText(LPCTSTR pstrText)
	{
		...
		if( m_pWindow != NULL ) Edit_SetText(*m_pWindow, sText);
		Invalidate();
	}
	...
	LPCTSTR CEditUI::GetNormalHasTextImage()
	{
		return m_sNormalHasTextImage;
	}

	void CEditUI::SetNormalHasTextImage(LPCTSTR pStrImage)
	{
		m_sNormalHasTextImage = pStrImage;
		Invalidate();
	}
	...
	{
		if( m_sTipValue == pStrTipValue ) return;

		m_sTipValue	= pStrTipValue;

		CDuiString* pStr = new CDuiString;
		if(CPaintManagerUI::GetLangText(pStrTipValue, *pStr))
		{
			if(m_LangMap.Find(pStrTipValue))
			{
				pStr = (CDuiString*)m_LangMap.Set(pStrTipValue, pStr);
			}
			else
			{
				if(m_LangMap.Insert(pStrTipValue, pStr))
					pStr = NULL;			
			}
		}
		if(pStr)
	...
	{
		CDuiString* pStrText = (CDuiString*)m_LangMap.Find(m_sTipValue);
		if(pStrText)
			return (*pStrText).GetData();
	...
	void CEditUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("normalhastextimage")) == 0 ) SetNormalHasTextImage(pstrValue);
	...
	void CEditUI::PaintStatusImage(HDC hDC)
	{
		...
		if( !m_sNormalImage.IsEmpty() ) {
			...
			if(!str.IsEmpty() && !m_sNormalHasTextImage.IsEmpty())
			{
				if( !DrawImage(hDC, (LPCTSTR)m_sNormalHasTextImage) ) m_sNormalHasTextImage.Empty();
				else return;
			}
			else
			{
	...
	void CEditUI::PaintText(HDC hDC)
	{
		...
		CDuiString sText;
		if(GetText() == GetTipValue() || GetText() == _T(""))	
		{
