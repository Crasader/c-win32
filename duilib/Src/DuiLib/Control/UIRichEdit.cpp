...
DWORD CRichEditUI::GetDefaultCharFormat(CHARFORMAT &cf) const
{
	cf.cbSize = sizeof(CHARFORMAT2);
	LRESULT lResult;
	TxSendMessage(EM_GETCHARFORMAT, 0, (LPARAM)&cf, &lResult);
...
bool CRichEditUI::SetDefaultCharFormat(CHARFORMAT &cf)
{
	if( !m_pTwh ) return false;
	cf.cbSize = sizeof(CHARFORMAT2);
	LRESULT lResult;
...
DWORD CRichEditUI::GetSelectionCharFormat(CHARFORMAT &cf) const
{
	cf.cbSize = sizeof(CHARFORMAT2);
	LRESULT lResult;
	TxSendMessage(EM_GETCHARFORMAT, 1, (LPARAM)&cf, &lResult);
...
bool CRichEditUI::SetSelectionCharFormat(CHARFORMAT &cf)
{
	if(m_pManager->IsBackgroundTransparent())
	{
		CRenderEngine::CheckAlphaColor(cf.crTextColor);
...
bool CRichEditUI::CanPaste(UINT nFormat)
{
	if( !m_pTwh ) return false;
	LRESULT lResult;
