...
CControlUI::~CControlUI()
{
	...
	for(int i = 0; i < m_LangMap.GetSize(); ++i)
	{
		LPCTSTR psKey = m_LangMap.GetAt(i);
		if(psKey)
		{
			CDuiString* pStr = (CDuiString*)m_LangMap.Set(psKey, NULL);
			if(pStr)
				delete pStr;
		}
	}
...
{
	CDuiString* pStrText = (CDuiString*)m_LangMap.Find(m_sText);
	if(pStrText)
		return *pStrText;
    return m_sText;
}
...
void CControlUI::SetText(LPCTSTR pstrText)
{
	...
	if(CPaintManagerUI::GetLangText(pstrText, *pStr))
	{
		if(m_LangMap.Find(pstrText))
		{
			pStr = (CDuiString*)m_LangMap.Set(pstrText, pStr);
		}
		else
		{
			if(m_LangMap.Insert(pstrText, pStr))
				pStr = NULL;			
		}
	}
	if(pStr)
...
{
	CDuiString* pStr = (CDuiString*)m_LangMap.Find(m_sToolTip);
	if(pStr)
		return *pStr;
...
void CControlUI::OnSwitchLang()
{
	for(int i = 0; i < m_LangMap.GetSize(); ++i)
	{
		LPCTSTR lpsKey = m_LangMap.GetAt(i);
		if(lpsKey)
		{
			CDuiString* pStr = new CDuiString;
			if(CPaintManagerUI::GetLangText(lpsKey, *pStr))
			{
				pStr = (CDuiString*)m_LangMap.Set(lpsKey, (LPVOID)pStr);
				Invalidate();
			}
			if(pStr)
				delete pStr;
