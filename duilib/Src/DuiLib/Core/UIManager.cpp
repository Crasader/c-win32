...
namespace DuiLib {
	...
	CDuiLangManager CPaintManagerUI::s_langMgr;
	...
	bool CPaintManagerUI::SetLangXml(LPCTSTR lpsLangXml, LPCTSTR lpsLangPath)
	{
		if(NULL == lpsLangXml || NULL == lpsLangPath)
			return false;

		bool bRtn = s_langMgr.SetLangPath(lpsLangPath);
		bRtn = bRtn && s_langMgr.SetLangXml(lpsLangXml);

		return bRtn;
	}

	bool CPaintManagerUI::SetDefLang(LPCTSTR lpsLang)
	{
		return s_langMgr.SetDefaultLang(lpsLang);
	}

	bool CPaintManagerUI::SwitchLang(LPCTSTR lpsLang, LPCTSTR lpsLangXml/* = NULL*/)
	{
		if(lpsLangXml && !s_langMgr.SetLangXml(lpsLangXml))
			return false;
	
		if(s_langMgr.SwitchLang(lpsLang))
		{
			for( int i = 0; i < m_aPreMessages.GetSize(); i++ )
			{
				CPaintManagerUI* pManager = static_cast<CPaintManagerUI*>(m_aPreMessages[i]);
	...
	void CPaintManagerUI::OnSwitchLang(CControlUI* lpItem)
	{
		if(NULL == lpItem)
			return ;

		IContainerUI* pCon = (IContainerUI*)lpItem->GetInterface(L"IContainer");
		if(pCon)
		{
			for(int i = 0; i < pCon->GetCount(); ++i)
			{
				CControlUI* pItem = pCon->GetItemAt(i);
				if(pItem)
					OnSwitchLang(pItem);
			}
		}
	...
	bool CPaintManagerUI::GetLangText(LPCTSTR lpsLangID, CDuiString& strText)
	{
		return s_langMgr.GetText(lpsLangID, strText);
	}
		...
		case WM_APP + 1:
			{
				...
				{
					//如果删除的是点击控件 则清空
					if(m_aDelayedCleanup[i]==m_pEventClick)
						m_pEventClick=NULL;
					if(m_aDelayedCleanup[i]==m_pFocus)
						m_pFocus=NULL;
					if(m_aDelayedCleanup[i]==m_pEventHover)
						m_pEventHover=NULL;
					if(m_aDelayedCleanup[i]==m_pEventKey)
						m_pEventKey=NULL;
	...
	void CPaintManagerUI::SetFocus(CControlUI* pControl)
	{
		...
		if( hFocusWnd != m_hWndPaint && pControl != m_pFocus &&pControl!=NULL ) 
			::SetFocus(m_hWndPaint);
		// Already has focus?
	...
	void CPaintManagerUI::ShowToolTip(LPCTSTR lpszText,RECT rc)
	{
		...
		m_ToolTip.uId = (UINT_PTR) m_hWndPaint;
		m_ToolTip.hinst = m_hInstance;
		m_ToolTip.lpszText = const_cast<LPTSTR>( lpszText);
		m_ToolTip.rect = rc;
		if( m_hwndTooltip == NULL ) {
			m_hwndTooltip = ::CreateWindowEx(0, TOOLTIPS_CLASS, NULL, WS_POPUP | TTS_NOPREFIX | TTS_ALWAYSTIP, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, m_hWndPaint, NULL, m_hInstance, NULL);
			::SendMessage(m_hwndTooltip, TTM_ADDTOOL, 0, (LPARAM) &m_ToolTip);
